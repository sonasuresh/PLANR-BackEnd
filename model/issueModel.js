...
async function updateIssue(i_id, i_title, i_desc) {
    try {
        if (typeof (i_id) == 'undefined') {
            throw new Error('Issue Id is Null!Cannot update issues');
        }
        if (typeof (i_title) == 'undefined' && typeof (i_desc) == 'undefined') {
            throw new Error('Issue Title and Issue Description is Null!');
        }
        let where = [];
        let queryParams = [];
